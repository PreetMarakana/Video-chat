...
function init() {
	...
	signalingSocket.on("addPeer", function(config) {
		...
		peerConnection.onaddstream = function(event) {
			...
			// muteb.id="myTag"; 
			// my.appendChild(muteb);	
			// const mutec=document.createElement("button");
			// mutec.className="fas fa-video" ;
		
			// mutec.id="myTag1"; 
			// my.appendChild(mutec);
			const remoteMedia = document.createElement("video");
			// videoWrap.appendChild(my);
			...
			//console.log(peerMediaElements[peer_id]);	
...
function setup_local_media(callback, errorback) {
	...
	navigator.mediaDevices
		...
		.then((stream) => {
			var a=document.getElementById("allowaccess").style.display = "none";
...
const resizeVideos = () => {
	...
	var myDiv = document.querySelector("#myVideoWrap");
	var fooEls = myDiv.querySelectorAll(":scope > .mirror");
	fooEls.item(0).className = "mirror " + numToString[videos.length];
